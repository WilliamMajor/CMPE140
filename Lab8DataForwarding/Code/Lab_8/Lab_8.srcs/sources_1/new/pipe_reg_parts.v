...
module pipe_reg_E(
		...
		output wire			branchE_out,
		output wire	[1:0]	reg_dstE_out,
		output wire [31:0]	pc_plus4E_out,
		output wire [31:0] 	instrE_out,
		output wire [31:0]	seE_out,
		output wire [31:0] 	rd1E_out,
		output wire [31:0]	rd2E_out
	);
	reg 		alu_srcE, dm2regE, mult_enE, we_dmE, pc_srcE, we_regE, branchE;
	reg [1:0]	reg_dstE;
	reg	[2:0]	alu_ctrlE;
	reg [31:0]	pc_plus4E, instrE, seE, rd1E, rd2E;
	always @ (posedge clk, posedge rst)
	begin
		if(rst == 1)
		begin
...
module pipe_reg_M(
		...
		output wire			branchM_out
	);
	reg [63:0] 	multM;
	reg [31:0] 	alu_outM, wd_dmM, pc_plus_brM;
	reg [4:0]	rf_waM;
	reg	dm2regM, mult_enM, pc_srcM, we_regM, zeroM, branchM;
	always @ (posedge clk, posedge rst)
	begin
		if(rst == 1)
		begin
